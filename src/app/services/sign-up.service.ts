...
export class SignUpService {
  ...
  createNewUser(requestParam) {
    ...
    if(requestParam) {
      ...
      return this.http.post( environment.baseUrl+'/createUser', body , options ).map(data => data.json()).catch(this.catch);
    }
  }
/********************* ends ***************/
...
getRegisteredUserByEmail(email) {
  if(email)
  {
     return this.http.get( environment.baseUrl + '/getUserByEmail?email='+email).map(data => data.json()).catch(this.catch);
  }
}
/******************* ends *****************/
...
validateUserPassword(object) {
  if(object)
  {
    ...
    let body = new URLSearchParams();
     ...
     return this.http.post(environment.baseUrl+'/validatePassword', body, options).map(data => data.json()).catch(this.catch);
  }
}
  /***************** catch error *****/
  catch(error) {
