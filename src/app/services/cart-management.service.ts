...
postItemToCustomerCart(requestBody) {
  if(requestBody)
  {
    ...
    return this.http.post( environment.baseUrl+'/postItem', body, options).map(data => data.json()).catch(this.catch);
  }
}
/*********************** ends ***************/
...
getCustomerCart(token) {
  if(token)
  {
    return this.http.get( environment.baseUrl+'/getUserCart?token='+token).map(data => data.json()).catch(this.catch);
  }
}
/***************** ends **********************/
...
postItemToCustomerHistory(requestBody) {
  if(requestBody)
  {
    ...
    return this.http.post( environment.baseUrl+'/postItemInHistory', body, options).map(data => data.json()).catch(this.catch);
  }
}
/*********************** ends ***************/
...
getCustomerHistory(token) {
  if(token)
  {
    return this.http.get( environment.baseUrl+'/getUserOrderHistory?token='+token).map(data => data.json()).catch(this.catch);
  }
}
/***************** ends **********************/
